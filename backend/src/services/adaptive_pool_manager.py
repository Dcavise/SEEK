...
class AdaptivePoolManager:
    ...
    def register_pool(self, pool_name: str, config: Optional[AdaptivePoolConfig] = None):
        """
    ...
    async def _monitoring_loop(self):
        ...
        while self._running:
            try:
                ...
                await asyncio.sleep(self.configs[list(self.configs.keys())[0]].evaluation_interval_seconds)
    ...
    async def _get_pool_metrics(self, pool_name: str) -> Optional[PoolMetrics]:
        """
    ...
    def _determine_scaling_action(
        ...
        if (
            ...
            and (current_metrics.wait_time_ms > 100 or current_metrics.error_rate > 0.05)
        ):
            return "scale_up"
        ...
        if (
            ...
            if len(recent_metrics) >= 5:
                avg_utilization = sum(m.utilization_percent for m in recent_metrics) / len(recent_metrics) / 100.0
    ...
    def get_scaling_recommendations(self) -> Dict[str, Dict]:
        ...
        for pool_name, config in self.configs.items():
            ...
            if len(history) >= 5:
                ...
                trend = "increasing" if recent_utilization[-1] > recent_utilization[0] else "decreasing"
            else:
                trend = "stable"
            ...
            recommendations[pool_name] = {
                ...
                "recommended_action": self._determine_scaling_action(pool_name, latest_metrics, config),
