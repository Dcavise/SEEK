...
class ConcurrencyManager:
    ...
    def __init__(self, limits: ConcurrencyLimits | None = None):
        ...
        self.semaphores: dict[OperationType, asyncio.Semaphore] = {
            OperationType.BULK_PROPERTY_LOOKUP: asyncio.Semaphore(
                self.limits.bulk_property_lookup
            ),
            OperationType.BULK_COMPLIANCE_SCORING: asyncio.Semaphore(
                self.limits.bulk_compliance_scoring
            ),
    ...
    async def execute_operation(
        ...
        **kwargs,
        ...
        async with semaphore:
            logger.debug(
                f"Executing {operation_type.value} (semaphore: {semaphore._value})"
            )
    ...
    async def _handle_bulk_property_lookup(self, operation: QueuedOperation):
        ...
        logger.info(
            f"Processing bulk property lookup for {len(coordinates)} coordinates"
        )
    ...
    async def _handle_bulk_compliance_scoring(self, operation: QueuedOperation):
        ...
        logger.info(
            f"Processing bulk compliance scoring for {len(property_ids)} properties"
        )
    ...
    async def _handle_etl_processing(self, operation: QueuedOperation):
        ...
        logger.info(
            f"Processing ETL for dataset: {dataset_info.get('type', 'unknown')}"
        )
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        semaphore_status = {
            op_type.value: {
                ...
                "utilization": (semaphore._bound_value - semaphore._value)
                / semaphore._bound_value
                * 100,
            }
        ...
        recent_completions = [
            ...
            if op.completed_at
            and op.completed_at > datetime.utcnow() - timedelta(hours=1)
        ]
