...
class RequireScopes:
    ...
    def __call__(
        ...
        if not all(scope in current_user.scopes for scope in self.required_scopes):
            missing_scopes = [
                s for s in self.required_scopes if s not in current_user.scopes
            ]
...
def create_tokens_for_user(user: UserInDB) -> JWTTokens:
    ...
    token_data = {"sub": user.username, "user_id": user.id, "scopes": user.scopes}
...
def check_rate_limit(request: Request, identifier: str) -> bool:
    ...
    if identifier in _rate_limit_store:
        _rate_limit_store[identifier] = [
            req_time
            for req_time in _rate_limit_store[identifier]
