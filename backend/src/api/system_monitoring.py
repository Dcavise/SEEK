...
from ..core.security import require_monitoring, require_monitoring_admin, TokenData
...
async def get_system_status(
    ...
    try:
        ...
        semaphore_utilizations = [
            status["utilization"] for status in concurrency_status["semaphore_status"].values()
        ]
        avg_utilization = sum(semaphore_utilizations) / len(semaphore_utilizations) if semaphore_utilizations else 0
...
async def register_adaptive_pool(
    ...
    target_utilization: float = Query(0.7, ge=0.1, le=0.9, description="Target utilization (0.1-0.9)"),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to register adaptive pool: {str(e)}"
        )
...
async def get_performance_metrics(
    ...
    try:
        ...
        metrics = {
            ...
            "system_efficiency": {
                "throughput_score": min(100, max(0, 100 - concurrency_status["queued_operations"])),
        ...
        if semaphore_utilizations:
            metrics["system_efficiency"]["resource_utilization"] = sum(semaphore_utilizations) / len(semaphore_utilizations)
...
async def restart_system_services(
    service: str = Query(..., description="Service to restart (concurrency|adaptive-pooling|all)"),
    current_user: TokenData = Depends(require_monitoring_admin),
) -> Dict[str, Any]:
    """
