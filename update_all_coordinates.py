...
class CoordinateUpdater:
            ...
            try:
                ...
                for search_term in search_terms:
                    ...
                    try:
                        ...
                        query_result = client.from("parcels").select("id, address").ilike("address", f"%{search_term}%").is_("latitude", "null").limit(5).execute()
    ...
    def get_database_stats(self):
        ...
        try:
            # Total parcels
            total_result = client.from("parcels").select("*", count="exact", head=True).execute()
            self.stats['total_parcels'] = total_result.count or 0
            
            # Parcels with coordinates
            coords_result = client.from("parcels").select("*", count="exact", head=True).not_("latitude", "is", None).not_("longitude", "is", None).execute()
